...
&xrArr; Inserir textos tachados é possível ao inserir a mensagem entre `~~`:
...
&xrArr; Além do alinhamento padrão à esquerda, também pode ser definido como centralizado, inserindo `:` no inicio e no final da linha que indica a coluna desejada, ou à esquerda, inserindo `:` no final da linha que indica a coluna desejada:
